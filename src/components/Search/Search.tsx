...
export const Search = () => {
  ...
  const handleOnSubmitPython = (event: React.FormEvent) => {
    event.preventDefault();
    if (process){
      console.log("Handling python");
      getMovies(process).then((movies) => {
        console.log("Got back " + movies.length + " movie items");
        updateMovies(movies);
      });
    }
  ...
  return (
    <div>
      ...
      <form name="form" onSubmit={(e) => handleOnSubmitPython(e)} noValidate>
        <input
          type="text"
          name="movie"
          className="search__input"
